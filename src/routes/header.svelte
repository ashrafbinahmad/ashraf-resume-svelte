...
>
	<nav
		class="container mx-auto mt-2 text-xl flex justify-between items-center h-[5rem] relative overflow-hidden
			...
			>
				{#each menuItems as item}
					<a href="#{item}" on:click={toggleMenu}>
						<li
							class="inline-block px-4 py-2 cursor-pointer relative z-20 transition-all max-sm:block max-sm:text-right"
						>
							{item}
						</li>
					</a>
